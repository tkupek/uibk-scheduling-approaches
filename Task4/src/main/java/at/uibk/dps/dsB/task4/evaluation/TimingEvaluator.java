...
public class TimingEvaluator implements ImplementationEvaluator {
	...
	private final PropertyProviderDynamic propertyProvider;

	@Inject
	public TimingEvaluator(PropertyProviderDynamic propertyProvider) {
	...
	private double calculateMakespan(Specification implementation) {
		...
		return endTimes.values().stream().mapToDouble(e -> e).max().orElse(0D);
	}

	/**
	 * Recursive depth first graph traversal calculate end times for all nodes in
	 * the graph
	 */
	private HashMap<Task, Double> getEndTimes(Application<Task, Dependency> application, Task task,
			Specification implementation, HashMap<Task, Double> startTimes, HashMap<Task, Double> endTimes) {
	...
	private double getTaskTime(Task task, Mappings<Task, Resource> resourceMappings) {
		...
		double executionTime = propertyProvider.getExecutionTime(taskResource);
		...
		if (PropertyService.isCloudResource(resource)) {
			availableResources = propertyProvider.getNumberOfAvailableInstances(resource);
		}
	...
	private double getTransmissionTime(Communication comm, Architecture<Resource, Link> routing) {
		double transmissionTimeOneMessage = routing.getEdges().stream()
				.mapToDouble(e -> propertyProvider.getTransmissionTime(comm, e)).sum();

		return transmissionTimeOneMessage * this.getNumberOfInstances(comm);
	}

	/**
	 * Tasks annotated with ITERATIVE_PEOPLE or ITERATIVE_CARS have to be executed
	 * once for each instance of the respective object in the processed frame
	 */
